"use client";
import { AngleRightIcon } from "@/assets/angle-right";
import { XIcon } from "@/assets/x-icon";
import { cn } from "@/utils/cn";
import {
  ValidationError as ServerError,
  useForm as useFormspree,
} from "@formspree/react";
import { motion } from "framer-motion";
import { useForm } from "react-hook-form";
import { Button } from "../button";
import { useModal } from "../modal";
import { FormInput } from "./form-input";
import { FormTextarea } from "./form-textarea";
import { SuccessMessage } from "./success-message";

type FormData = {
  firstName: string;
  lastName: string;
  email: string;
  website: string;
  message: string;
};

export const ContactForm = () => {
  const { hideModal } = useModal();

  const [serverState, sendToFormspree] = useFormspree("xeojzovp");
  const {
    register,
    handleSubmit,
    reset,
    formState: { errors },
  } = useForm<FormData>();

  return (
    <motion.div
      initial={{ opacity: 0, y: "100%" }}
      animate={{ opacity: 1, y: 0 }}
      exit={{ opacity: 0, y: "100%" }}
      transition={{ duration: 0.3 }}
      className="p-4 pb-8 rounded-xl bg-neutral-700 w-full max-w-[700px] mx-auto drop-shadow-lg shadow-neutral-100 border border-neutral-400/80 max-h-[90svh] overflow-auto"
    >
      <div className="flex justify-end">
        <Button
          onClick={hideModal}
          className="bg-neutral-400 p-3 rounded-md hover:bg-neutral-300"
          aria-label="Close modal"
          title="Close modal"
        >
          <XIcon width={14} height={14} />
        </Button>
      </div>
      {serverState.succeeded ? (
        <SuccessMessage />
      ) : (
        <>
          <h2 className="text-3xl font-black font-suwannaphum mb-3">
            Let's Talk
          </h2>
          <span className="text-secondary-300 font-medium">
            Send us your inquiry and our support will contact you soon.
          </span>
          <form onSubmit={handleSubmit(sendToFormspree)} className="grid mt-6">
            <div className="grid gap-5">
              <FormInput
                label="Name"
                name="name"
                register={register}
                errors={errors}
              />

              <FormInput
                label="Email"
                name="email"
                register={register}
                errors={errors}
              />

              <FormInput
                label="Website"
                name="website"
                register={register}
                errors={errors}
              />
            </div>

            <div className="py-4">
              <FormTextarea
                label="Message"
                name="message"
                register={register}
                errors={errors}
              />
            </div>

            <Button
              disabled={serverState.submitting}
              className={cn(
                "rounded-md py-3 px-6 text-secondary-800 font-semibold bg-primary-100",
                "flex items-center justify-center gap-2",
                "hover:bg-primary-300 active:bg-primary-400",
                "group",
              )}
            >
              {serverState.submitting ? (
                <div role="status" className="flex items-center gap-4">
                  <svg
                    aria-hidden="true"
                    className="w-6 h-6 text-gray-200 animate-spin fill-green-300"
                    viewBox="0 0 100 101"
                    fill="none"
                  >
                    <path
                      d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                      fill="currentColor"
                    />
                    <path
                      d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                      fill="currentFill"
                    />
                  </svg>
                  <span>Sending</span>
                </div>
              ) : (
                <span className="flex items-center gap-2">
                  Send
                  <span
                    className={cn(
                      "block transition-transform",
                      "group-hover:translate-x-1",
                      "group-focus-visible:translate-x-1",
                    )}
                  >
                    <AngleRightIcon />
                  </span>
                </span>
              )}
            </Button>

            <ServerError
              className="text-red-500 p-2 text-center"
              errors={serverState.errors}
            />
          </form>
        </>
      )}
    </motion.div>
  );
};
